...
hdao_snapshot["Balance"] = (1e6 * hdao_snapshot["Balance"]).astype(int)

# Remove rows with zero balance
hdao_snapshot = hdao_snapshot[hdao_snapshot.Balance > 0]

# Sort the users by their hDAO balance
hdao_snapshot = hdao_snapshot.sort_values(by="Balance", ascending=False)

# Transform the data frame into a python dictionary
hdao_snapshot = hdao_snapshot.to_dict()["Balance"]
